...
func ListDetail(ctx context.Context, req *pbkafka.ListDetailReq) (*pbkafka.ListDetailResp, error) {
	var cli kafkaer.Kafkaer
	...
	for _, tn := range tenanters {
		if req.AccountName == "" || tn.AccountName() == req.AccountName {
			if cli, err = kafkaer.NewKafkaClient(req.Provider, region, tn); err != nil {
				return nil, errors.WithMessage(err, "NewKafkaClient error")
			}
			break
		}
	}
	...
	if cli == nil {
		return nil, errors.Errorf("no kafka client for provider %v account %q", req.Provider, req.AccountName)
	}

	return cli.ListDetail(ctx, req)
}

func List(ctx context.Context, req *pbkafka.ListReq) (*pbkafka.ListResp, error) {
	var (
		wg     sync.WaitGroup
		mutex  sync.Mutex
		kafkas []*pbkafka.KafkaInstance
	)
	...
	for _, t := range tenanters {
		for _, region := range regions {
			go func(tenant tenanter.Tenanter, region tenanter.Region) {
				defer wg.Done()
				cli, err := kafkaer.NewKafkaClient(req.Provider, region, tenant)
				if err != nil {
					glog.Errorf("NewKafkaClient error %v", err)
					return
				}

				request := &pbkafka.ListDetailReq{
				...
				for {
					...
					if err != nil {
						glog.Errorf("kafka ListDetail error %v", err)
						return
					}
					mutex.Lock()
					kafkas = append(kafkas, resp.Kafkas...)
					mutex.Unlock()
					if resp.Finished {
						break
					}
	...
	return &pbkafka.ListResp{Kafkas: kafkas}, nil
}
