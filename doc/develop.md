...
使用 **Docker / docker compose** 构建后端镜像时，Dockerfile 内会先 **`buf mod update`** 再 **`buf generate`**（拉取 buf.yaml 中的依赖），仅修改 `idl/` 也可完成构建；本地开发仍建议安装 buf 后运行 `./gen.sh` 提交与 `idl` 一致的 `gen/`。建议将生成的 **`buf.lock` 一并提交**，便于可复现构建。
